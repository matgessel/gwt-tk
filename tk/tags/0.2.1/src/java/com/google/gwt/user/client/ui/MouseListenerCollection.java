 ...
 * correct mouse coordinates in scrolled documents. Coordinates are given
 * relative to source widget's upper-left corner.
 * </p>
 */
	...
	public void fireMouseEvent(Widget sender, Event event)
	{
		...
		switch (DOM.eventGetType(event))
		{
			...
			case Event.ONMOUSEOVER:
				// Only fire the mouseEnter event if it's coming from outside this widget.
				Element from = DOM.eventGetFromElement(event);
				if (!DOM.isOrHasChild(sender.getElement(), from))
					fireMouseEnter(sender);
				break;
			case Event.ONMOUSEOUT:
				// Only fire the mouseLeave event if it's actually leaving this widget.
